...
export class GroupService {
    ...
    static async sendMessage(input: MessageInput, userId: string): Promise<MessageOutput> {
        const { groupId, content, type = "TEXT", mediaUrl, imageUrl } = input

        // Allow image-only messages
        if (type === "TEXT" && (!content || !content.trim()) && !imageUrl && !mediaUrl) {
        ...
        const message = await prisma.groupMessage.create({
            data: {
                ...
                content: content || "",
                type: type as any,
                mediaUrl: imageUrl || mediaUrl,  // imageUrl takes precedence
    ...
    static async getGroupMessages(groupId: string, userId: string, cursor?: string, limit: number = 20) {
        ...
        if (!cursor) {
            try {
                ...
                if (cached && cached.length > 0) {
                    ...
                    // Filter for View Once
                    const filteredMessages = messages.map(msg => {
                        const hasViewed = msg.viewedBy?.some((u: any) => u.id === userId);
                        const canView = msg.isViewOnce && !hasViewed && msg.senderId !== userId;
                        return {
                            ...msg,
                            mediaUrl: (msg.isViewOnce && !canView) ? null : msg.mediaUrl
                        };
                    });

                    return { messages: filteredMessages, nextCursor, isMuted: membership.isMuted }
                }
            } catch (e) { }
        }
        ...
        return {
            messages: returnMessages.map(msg => {
                const m = msg as any;
                const hasViewed = m.viewedBy?.some((u: any) => u.id === userId);
                const canView = m.isViewOnce && !hasViewed && m.senderId !== userId;
                return {
                    id: m.id,
                    groupId: m.groupId,
                    senderId: m.senderId,
    ...
    static async markGroupMessageViewed(messageId: string, userId: string) {
        const message = await prisma.groupMessage.findUnique({
            where: { id: messageId }
        });
        if (!message) throw new Error("MESSAGE_NOT_FOUND");
